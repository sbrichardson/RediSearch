...
def testAddErrors(env):
    env.expect('ft.create idx EXPRESSION prefix("",@__key) schema foo text bar numeric sortable').equal('OK')
...
def testConditionalUpdate(env):
    env.assertOk(env.cmd(
        'ft.create', 'idx','EXPRESSION', 'prefix("doc", @__key)', 
...
def testUnionIdList(env):
    ...
    N = 100
    env.assertOk(r.execute_command(
        "ft.create", "test", 'EXPRESSION', 'prefix("", @__key)', 
...
def testAttributes(env):
    env.assertOk(env.cmd('ft.create', 'idx','EXPRESSION', 'prefix("doc", @__key)', 
...
def testUnion(env):
    ...
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx','EXPRESSION', 'prefix("doc", @__key)', 'schema', 'f', 'text'))
    for i in range(N):
...
def testDrop(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'f', 'text', 'n', 'numeric', 't', 'tag', 'g', 'geo'))
...
def testCustomStopwords(env):
    ...
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'foo', 'text'))

    # Index with custom stopwords
    env.assertOk(r.execute_command('ft.create', 'idx2', 'EXPRESSION', 'prefix("doc", @__key)', 'stopwords', 2, 'hello', 'world',
                                    'schema', 'foo', 'text'))
    ...
    env.assertOk(r.execute_command('ft.create', 'idx3', 'EXPRESSION', 'prefix("doc", @__key)', 'stopwords', 0,
                                    'schema', 'foo', 'text'))
...
def testStopwords(env):
    ...
    # made earlier
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'stopwords', 3, 'foo',
...
def testNoStopwords(env):
    ...
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'title', 'text')
    for i in range(100):
...
def testOptional(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'foo', 'text'))
...
def testExplain(env):
    ...
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)',
        'schema', 'foo', 'text', 'bar', 'numeric', 'sortable'))
...
def testNoIndex(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema',
        'foo', 'text',
...
def testPaging(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'foo', 'text', 'bar', 'numeric', 'sortable'))
    N = 100
    for i in range(N):
...
def testSortBy(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'foo', 'text', 'sortable', 'bar', 'numeric', 'sortable'))
    N = 100
    for i in range(N):
        env.assertOk(r.execute_command('ft.add', 'idx', 'doc%d' % i, 1.0, 'fields',
...
def testSlopInOrder(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'title', 'text'))
...
def testGeoErrors(env):
    env.expect('flushall')
    env.expect('ft.create idx EXPRESSION prefix("",@__key) schema name text location geo').equal('OK')
...
def testGeo(env):
    ...
    env.assertOk(r.execute_command('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)',
...
def testTagErrors(env):
    env.expect("ft.create", "test", 'EXPRESSION', 'prefix("", @__key)', 
...
def testGeoDeletion(env):
    ...
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema',
...
def testStemming(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'title', 'text'))
    env.assertOk(r.execute_command('ft.add', 'idx', 'doc1', 0.5, 'fields',
                                    'title', 'hello kitty'))
    env.assertOk(r.execute_command('ft.add', 'idx', 'doc2', 1.0, 'fields',
                                    'title', 'hello kitties'))
...
def testNumericRange(env):
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'title', 'text', 'score', 'numeric', 'price', 'numeric'))
...
def testReturning(env):
    env.assertCmdOk('ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema',
                     'f1', 'text',
                     'f2', 'text',
                     'n1', 'numeric', 'sortable',
                     'f3', 'text')
...
def _test_create_options_real(env, *options):
    ...
    options = ['idx'] + options + ['EXPRESSION', 'prefix("doc", @__key)', 'schema', 'f1', 'text', 'f2', 'text']
...
def testInfoCommand(env):
    ...
    r = env
    env.assertOk(r.execute_command(
        'ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'NOFIELDS', 'schema', 'title', 'text'))
    N = 50
    for i in xrange(N):
    ...
    for x in range(1, 5):
        for combo in combinations(('NOOFFSETS', 'NOFREQS', 'NOFIELDS', ''), x):
            ...
            try:
                ...
                pass
            env.assertCmdOk('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', *options)
...
def testNoStem(env):
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)',
...
def testSortbyMissingField(env):
    # GH Issue 131
    env.cmd('ft.create', 'ix', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'txt',
...
def testParallelIndexing(env):
    # GH Issue 207
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'txt', 'text')
...
def testNonDefaultDb(env):
    ...
    # Should be ok
    env.cmd('FT.CREATE', 'idx1', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'txt', 'text')
    try:
        env.cmd('SELECT 1')
    except redis.ResponseError:
        return
    ...
    # Should fail
    with env.assertResponseError():
        env.cmd('FT.CREATE', 'idx2', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'txt', 'text')
...
def testLuaAndMulti(env):
    ...
    env.cmd('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'f1', 'text', 'n1', 'numeric')
...
def testLanguageField(env):
    env.cmd('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'language', 'TEXT')
    env.cmd('FT.ADD', 'idx', 'doc1', 1.0,
...
def testUninitSortvector(env):
    ...
    env.cmd('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'f1', 'TEXT')
    for x in range(2000):
...
def testAlterIndex(env):
    env.skip() #alter not supported 2.0
    env.cmd('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'f1', 'TEXT')
...
def testAlterValidation(env):
    ...
    env.cmd('FT.CREATE', 'idx1', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'f0', 'TEXT')
    for x in range(1, 32):
    ...
    env.cmd('FT.CREATE', 'idx2', 'EXPRESSION', 'prefix("doc", @__key)', 'MAXTEXTFIELDS', 'SCHEMA', 'f0', 'TEXT')
    ...
    env.cmd('FT.CREATE', 'idx3', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'f0', 'text')
...
def testIssue366_1(env):
    ...
    env.cmd('FT.CREATE', 'idx1', 'EXPRESSION', 'prefix("doc", @__key)',
...
def testIssue654(env):
    ...
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 'id', 'numeric')
...
def testReplaceReload(env):
    env.cmd('FT.CREATE', 'idx2', 'EXPRESSION', 'prefix("doc", @__key)',
            'SCHEMA', 'textfield', 'TEXT', 'numfield', 'NUMERIC')
...
def testIssue422(env):
    env.cmd('ft.create', 'myIdx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema',
...
def testTimeoutSettings(env):
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'schema', 't1', 'text')
...
def testSpellCheck(env):
    env.cmd('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'report', 'TEXT')
...
# 127.0.0.1:6379>  FT.AGGREGATE productSearch1 * load 2 @color @categoryid APPLY "split(format(\"%s-%s\",@color,@categoryid),\"-\")" as value GROUPBY 1 @value REDUCE COUNT 0 as value_count
    env.cmd('ft.create', 'productSearch1', 'noscoreidx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'productid',
...
def testIssue501(env):
    env.cmd('FT.CREATE', 'incidents', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'report', 'TEXT')
...
def testIssue589(env):
    env.cmd('FT.CREATE', 'incidents', 'EXPRESSION', 'prefix("doc", @__key)', 'SCHEMA', 'report', 'TEXT')
    env.cmd('FT.ADD', 'incidents', 'doc1', 1.0, 'FIELDS', 'report', 'report content')
    env.expect('FT.SPELLCHECK', 'incidents', 'report :').error().contains("Syntax error at offset")

def testIssue621(env):
    env.expect('ft.create', 'test', 'EXPRESSION', 'prefix("a", @__key)', 'SCHEMA', 'uuid', 'TAG', 'title', 'TEXT').equal('OK')
...
def testPrefixDeletedExpansions(env):
    ...
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'txt1', 'text', 'tag1', 'tag')
...
def testOptionalFilter(env):
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 't1', 'text')
    for x in range(100):
...
def testIssue736(env):
    ...
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 
...
def testIssue828(env):
    env.cmd('ft.create', 'beers', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA',
...
def testIssue862(env):
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'test', 'TEXT', 'SORTABLE')
...
def testIssue_884(env):
    env.expect('FT.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'STOPWORDS', '0', 'SCHEMA', 'title', 'text', 'weight',
...
def testIssue_848(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'test1', 'TEXT', 'SORTABLE').equal('OK')
...
def testMod_309(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'test', 'TEXT', 'SORTABLE').equal('OK')
...
def testIssue_865(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', '1', 'TEXT', 'SORTABLE').equal('OK')
...
def testIssue_779(env):
    ...
    env.cmd('FT.CREATE idx2 EXPRESSION prefix("",@__key) SCHEMA ot1 TAG')
...
def testUnknownSymbolErrorOnConditionalAdd(env):
    env.expect('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA f1 TAG f2 NUMERIC NOINDEX f3 TAG NOINDEX').ok()
...
def testWrongResultsReturnedBySkipOptimization(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 
                'SCHEMA', 'f1', 'TEXT', 'f2', 'TEXT').equal('OK')
...
def testAggregateSortByMaxNumberOfFields(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA',
               'test1', 'TEXT', 'SORTABLE',
               'test2', 'TEXT', 'SORTABLE',
               'test3', 'TEXT', 'SORTABLE',
               'test4', 'TEXT', 'SORTABLE',
...
def testNumericFilterError(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)',
                'SCHEMA', 'test', 'NUMERIC', 'SORTABLE').equal('OK')
...
def testGroupByWithApplyError(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'test', 'TEXT').equal('OK')
    env.expect('ft.add', 'idx', 'doc1', '1.0', 'FIELDS', 'test', 'foo').equal('OK')
...
def testTimeFormatError(env):
    env.expect('FT.CREATE', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'test', 'NUMERIC').equal('OK')
...
def testSortkeyUnsortable(env):
    env.cmd('ft.create', 'idx', 'EXPRESSION', 'prefix("", @__key)', 'schema', 'test', 'text')
...
def testIssue1085(env):
    env.skipOnCluster()
    env.cmd('FT.CREATE issue1085 EXPRESSION prefix("",@__key) SCHEMA foo TEXT SORTABLE bar NUMERIC SORTABLE')
    for i in range(1, 10):
...
def testSetPayload(env):
    env.skipOnCluster()
    env.expect('flushall')
    env.expect('ft.create idx EXPRESSION prefix("",@__key) schema name text').equal('OK')
...
def testIndexNotRemovedFromCursorListAfterRecreated(env):
    env.expect('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA f1 TEXT').ok()
    env.expect('FT.AGGREGATE idx * WITHCURSOR').equal([[0], 0])
    env.expect('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA f1 TEXT').error()
    env.expect('FT.AGGREGATE idx * WITHCURSOR').equal([[0], 0])

def testHindiStemmer(env):
    env.cmd('FT.CREATE', 'idxTest', 'EXPRESSION', 'prefix("", @__key)', 'SCHEMA', 'body', 'TEXT')
...
def testMOD507(env):
    env.skipOnCluster()
    env.expect('ft.create idx EXPRESSION prefix("",@__key) SCHEMA t1 TEXT').ok()
...
def testUnseportedSortableTypeErrorOnTags(env):
    env.skipOnCluster()
    env.expect('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA f1 TEXT SORTABLE f2 NUMERIC SORTABLE NOINDEX f3 TAG SORTABLE NOINDEX f4 TEXT SORTABLE NOINDEX').ok()
...
def testIssue1158(env):
    env.cmd('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA txt1 TEXT txt2 TEXT txt3 TEXT')
...
def testIssue1159(env):
    env.cmd('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA f1 TAG')
    for i in range(1000):
        env.cmd('FT.add idx doc%d 1.0 FIELDS f1 foo' % i)

def testIssue1169(env):
    env.cmd('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA txt1 TEXT txt2 TEXT')
...
def testIssue1184(env):
    ...
    for ft in field_types:
        env.assertOk(env.execute_command('FT.CREATE idx EXPRESSION prefix("",@__key) SCHEMA  field ' + ft))
